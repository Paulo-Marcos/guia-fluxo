...
3. **Nao edite arquivos gerados.** `plugins/guia/commands/<verbo>.md` (plugin command do Claude oficial, surge como `/guia:<verbo>`) e `plugins/guia/.agents/skills/<verbo>/SKILL.md` (cross-tool Codex+Antigravity, convencao AGENTS.md) sao saida de `core/build/render-skills.py` a partir de `core/manifest/manifest.yaml`. Mude o manifest e rode `python core/build/render-skills.py`. Edicao direta e sobrescrita na proxima render e quebra o `--check` da CI. Em `plugins/guia/.claude-plugin/plugin.json` (e `.claude-plugin/marketplace.json`) edite a mao so name/description/metadados; o campo `version` e propagado pelo render a partir do arquivo `VERSION` (raiz, **fonte unica** do numero - ADR-0019). Bump de versao = editar `VERSION` + rodar `python core/build/render-skills.py`. Decisao do layout em [`docs/adr/0006-plugin-oficial-claude-code.md`](docs/adr/0006-plugin-oficial-claude-code.md).
4. **Respeite o lock.** `.guia/locks/registry.yaml` lista arquivos travados. Se voce precisa editar um deles, a mensagem de commit precisa de `[unlock:<feature-id>] motivo: <razao curta>`. Receita: [`docs/how-to/editar-arquivo-travado.md`](docs/how-to/editar-arquivo-travado.md). Em sessoes Claude Code o hook **PreToolUse** (`core/hooks/hooks.json`) ja barra o `Edit`/`Write`/`MultiEdit` num arquivo travado **antes da edicao** (exit 2 com o motivo) - se isso acontecer, pare e peca autorizacao/unlock; o hook `commit-msg` rejeita o commit como segunda linha de defesa.
...
4. **Marque pronto:**
   ...
   ```
5. **Nao feche sozinho.** `finish` e acao **do usuario**: a IA so executa quando o desenvolvedor solicita (`/guia:finish`) ou autoriza explicitamente, e **nunca** dispara `finish` por conta propria. `finish` so depois de validacao humana em uso real - test suite passar **nao basta** - o usuario testa de verdade e ai pede o `finish`. Isto e **regra de comportamento** (esta documentada aqui, na skill e no CLAUDE.md), nao um parametro nem um gate tecnico no motor: o `finish` nao exige env nem flag (D-098 removeu o gate por env `GUIA_HUMAN_FINISH` que o D-080 havia tentado - mandar variavel era ruim e o motor nao consegue distinguir agente de humano). Default da IA: para no `ready` e entrega; aguarda o usuario disparar o `finish`.
