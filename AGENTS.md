...
1. **Identifique a demanda.** Rode `.\core\bin\ai.ps1 status` para ver a task atual. Se nao houver, abra:
   - Bug: `.\core\bin\ai.ps1 bug "Titulo" --context "Sintoma e impacto"` (substitui `issue`).
   - Capacidade nova: `.\core\bin\ai.ps1 feature "Titulo" --context "Motivo e escopo"`.
   - Manutencao (refactor pequeno, build, deps): `.\core\bin\ai.ps1 chore "Titulo" --context "O que e por que"`.
