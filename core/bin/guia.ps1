...
function Get-PythonCandidates {
    ...
    if ($env:GUIA_PYTHON) {
        $candidates.Add($env:GUIA_PYTHON) | Out-Null
    }
...
Sugestoes:
  - setar `$env:GUIA_PYTHON com o path completo
...
$script = Join-Path $PSScriptRoot "..\src\guia.py"

if (-not (Test-Path $script)) {
    throw "Motor nao encontrado em $script. Confirme que core/src/guia.py existe."
}
