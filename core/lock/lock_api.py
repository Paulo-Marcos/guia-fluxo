...
def _resolve_repo_root() -> Path:
    ...
    1. `GUIA_PROJECT_ROOT` env override (the commit-msg hook sets it to
       `git rev-parse --show-toplevel`; explicit and wins).
    2. Script-relative grandparent when it already holds `.guia/` — the
       source repo (`core/lock/`) and the dogfood build keep their historic
       behavior regardless of CWD.
    3. Caller's CWD otherwise. This is the plugin-global consumer (D-076):
       a copy of this module shipped inside the plugin would resolve
       `parents[2]` to the plugin dir, not the project, so the root must
       come from where the command runs. Both the engine (`finish --lock`,
       run with CWD=project) and the git commit-msg hook (run with CWD=repo
       root) land on the consumer repo here.
    """
