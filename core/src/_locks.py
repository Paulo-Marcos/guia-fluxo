...
In the dist/ layout (consumer), `ai.py` and `lock_api.py` sit side by
side under `dist/bin/`, so `import lock_api` just works. In the
repo-mae, `lock_api.py` lives in `core/lock/`, so we add that directory
